...
- **Dependency security**: dependency pins live in `package.json` `overrides` — that section is the canonical record, so don't maintain override docs anywhere else. Every entry is a deliberate, audit-driven decision; don't remove one without re-running the audit. The pre-push gate runs `npm run audit` at `--audit-level=high`: highs/criticals must be fixed or explicitly waived. Remediation workflow: `.agents/skills/npm-audit-fix/SKILL.md`.
